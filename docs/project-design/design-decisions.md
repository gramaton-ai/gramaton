...
(c) **Disable the task entirely.** Rejected as default — the task finds real contradictions when they exist, and keeping autonomous curation unbalanced (contradictions off, classification on) would be surprising. Operators who don't want the task can still set `llm.curation.contradiction.max_checks: 0`.
...
**Decision:** LLM configuration uses an `llm.models` block with three tiers — `low`, `medium`, `high` — and an `llm.models.tasks` map that assigns each task to a tier. Recognized tasks: `classification_short`, `classification_long`, `summarization`, `contradiction`, `concept`, `manifest`, `rerank`, `decompose`. Default assignments are Haiku-grade for short classification / summarization / manifest rollup / rerank / decompose, Sonnet-grade for long classification / contradiction detection / concept synthesis. (Original 2026-04 landing used a top-level `llm.model` field plus per-task `*_effort` fields under `llm_curation:`. The 2026-04-27 LLM-config consolidation removed `llm.model` entirely and merged the effort dials into a single `llm.models.tasks` map; behavior is identical.)
...
**Decision:** A session is a two-phase structure. Phase 1 is `gramaton_session_prepare`, which returns extraction instructions and the session state so far. Phase 2 is `gramaton_session_save`, which submits extracted segments. Each segment creates a Session segment node (BM25-indexed, part of the conversational thread) and, when `promote_to_memory: true` (the default), a linked Memory record (vector-embedded, full lifecycle, auto-supersession). Segments with `promote_to_memory: false` stay Session-only — searchable within session-scoped queries, not competing in Memory's vector space. An `extracted_as` edge links each Session segment to its Memory record.
