...
**Session binding is per working directory.** The session-start hook maps the client's session id to a Gramaton session in a per-cwd state file, which is what `gramaton session current` reads — and what the pre-compact archive hook resolves first (falling back to a global pointer only when the hook payload carries no working directory). The binding is last-writer-wins within one directory: two hooked harnesses running concurrently in the same working directory (say, Claude Code and Codex) contend for it, and the most recent session-start owns the binding. Concurrent sessions in *different* directories don't interact.
