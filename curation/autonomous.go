...
func generateManifestSummary(ctx context.Context, e *core.Engine, llmProv llm.Provider, cfg config.Config, result *AutonomousResult, cache *ManifestCache, logger *slog.Logger) {
	...
	for msIt.Next() {
		...
		// Concepts are a derived layer regenerated by curation, not
		// records the store holds on the caller's behalf. Counting
		// them inflates totalRecords and lets a synthesis pass alone
		// move the fingerprint, invalidating a manifest summary whose
