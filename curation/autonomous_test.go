...
// TestManifestRecordCountExcludesConcepts pins the derived-layer
// boundary in the store manifest. Concepts are regenerated by
// curation from the records they aggregate; counting them as records
// inflates the manifest's totals and lets a synthesis pass alone move
// the state fingerprint, throwing away a cached summary whose
// underlying record set never changed.
func TestManifestRecordCountExcludesConcepts(t *testing.T) {
	eng := setupEngine(t)
	for i := 0; i < 6; i++ {
		addProcessedNodeWithEmbedding(t, eng, fmt.Sprintf("Record %d about auth tokens", i),
			[]float32{float32(i) * 0.1, 0.5, 0.3})
	}

	cfg := config.Defaults()
	cache := &ManifestCache{}

	llm := &mockLLM{responses: []string{"Strong coverage of authentication and token management."}}
	first := &AutonomousResult{}
	generateManifestSummary(context.Background(), eng, llm, cfg, first, cache, nil)
	if first.ManifestSummary == "" || first.LLMCalls != 1 {
		t.Fatalf("first run: summary=%q calls=%d, want a summary from one LLM call",
			first.ManifestSummary, first.LLMCalls)
	}

	// A synthesis pass adds a concept over the same records.
	eng.Lock()
	concept := eng.Graph().AddNode(graph.Properties{
		"node_type":       graph.StringProperty("concept"),
		"concept_keyword": graph.StringProperty("auth"),
		"content_full":    graph.StringProperty("Records converge on auth tokens."),
		"created_at":      graph.TimestampProperty(time.Now().UTC()),
	})
	for k, v := range concept.Properties {
		eng.PropIdx().Add(concept.ID, k, v)
	}
	eng.Save("curation: concept emerge")
	eng.Unlock()

	second := &AutonomousResult{}
	llm2 := &mockLLM{responses: []string{"Should not be called."}}
	generateManifestSummary(context.Background(), eng, llm2, cfg, second, cache, nil)
	if !second.ManifestCacheHit {
		t.Fatal("a new concept moved the manifest fingerprint; concepts are derived, not records the manifest counts")
	}
	if second.LLMCalls != 0 {
		t.Fatalf("second run made %d LLM calls, want 0", second.LLMCalls)
	}
}
