...
// (machine-owned derived layer), matching Update and Classify: a
// record the caller cannot edit is one it cannot resolve either.
//
...
func (a *API) Resolve(ctx context.Context, req ResolveRequest) (ResolveResponse, *APIError) {
	...
	if kt, _ := target.Properties.GetString("knowledge_type"); kt == "segment" {
		return ResolveResponse{}, ErrInvalid("session segments are append-only; use gramaton_session_save to update capture status")
	}
	if graph.IsConcept(target.Properties) {
		return ResolveResponse{}, ErrInvalid("concept nodes are machine-owned derived summaries regenerated by curation; resolve the member records instead")
	}
	if graph.IsSectionOrChunk(target.Properties) {
		return ResolveResponse{}, ErrInvalid("section/chunk children are derived from their parent document; resolve the parent record instead")
	}
