...
func (a *API) sessionAddTopic(ctx context.Context, sessionID string, name string, branchedFrom string) (map[string]any, *APIError) {
	...
	if branchedFrom != "" {
		n, ok := a.engine.Graph().GetNode(branchedFrom)
		if !ok {
		...
		for _, e := range a.engine.Graph().EdgesFrom(branchedFrom) {
...
func (a *API) sessionAddSegment(ctx context.Context, sessionID string, topicID string, content string) (map[string]any, *APIError) {
	...
	topicNode, ok := a.engine.Graph().GetNode(topicID)
	if !ok {
	...
	belongsToSession := false
	for _, e := range a.engine.Graph().EdgesFrom(topicID) {
		if e.Type == "topic_of" && e.TargetID == sessionID {
			belongsToSession = true
			break
		}
	...
	segNode := a.engine.Graph().AddNode(props)
	// BM25-index the segment content (no vector embedding -- BM25-only per B1).
	a.engine.IndexNode(segNode.ID, content, nil)
	...
	a.log.Info("segment added", "component", "session", "session_id", sessionID,
		"topic_id", topicID, "segment_id", segNode.ID, "content_len", len(content))
	a.log.Debug("segment write timing", "component", "session", "segment_id", segNode.ID, "duration", dur)
...
func (a *API) sessionUpdateSegmentCapture(ctx context.Context, segmentID string, capturedAs string) (map[string]any, *APIError) {
	...
	n, ok := a.engine.Graph().GetNode(segmentID)
	if !ok {
	...
	now := time.Now().UTC()
	a.engine.SetProp(segmentID, "captured_as", graph.StringProperty(capturedAs))
