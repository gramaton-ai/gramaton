...
// (fresh, at the Step 0 route question that now leads), the identity
// enter-throughs ("", ""), and "5" (skip embedding) pre-scripted,
// then feeds the caller's Step 2 answers. Steps 3/4 are
// short-circuited via a fakeMCPBackend with no detected clients.
// Keeps tests fast and deterministic — no network, no real
// filesystem mutation beyond the tmpdir.
